...
import { loadData, saveData, getStorageMode, fmt, EMPTY_DATA, NAV, normalizeData } from './shared';
...
export default function App(){
  ...
  useEffect(()=>{(async()=>{const d=await loadData();setSmode(getStorageMode());setData(normalizeData(d));})();},[]);
  useEffect(()=>{
    if(!data)return;
    if(firstLoad.current){firstLoad.current=false;return;}
    saveData(data).then(ok=>setSmode(ok?(getStorageMode()==='cloud'?'cloud':getStorageMode()):'error'));
  },[data]);
  ...
  const upsert=(coll,rec)=>setData(d=>{
    const list=d[coll];const i=list.findIndex(x=>x.id===rec.id);
    const nl=i>=0?list.map(x=>x.id===rec.id?rec:x):[...list,rec];
    return{...d,[coll]:nl};
  });
  ...
  const shared={data,setData,cur,M,flash,upsert,remove,S,setSettings,projFilter,setProjFilter,setView,openProject,setOpenProject,smode};
  ...
  return(<div className="app">
    <aside className="sidebar no-print">
      ...
      <div className="sidebar-foot">
        <div className="save-chip"><span className={"led"+(smode==='memory'?' warn':'')}></span>
          {smode==='cloud'?'Synced securely':smode==='claude'?'Saved automatically':smode==='local'?'Saved on this device':smode==='error'?'Sync paused — export a backup':'Not saved — export to keep data'}</div>
      </div>
    </aside>
    <main className="main">
